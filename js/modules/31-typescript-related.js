      ...
      explanation: `
        <div class="analogy-box">
          ...
          <div class="analogy-body">
            ...
            <p class="analogy-text">Spell-check that runs before you hit send, not after the email has already landed in someone's inbox. TypeScript's strict mode and Angular's strict templates are that pre-send check — nullable state, mistyped inputs, and accidental <code>any</code> get flagged at build time instead of surfacing as a "cannot read property of undefined" crash in front of a real user.</p>
          </div>
        </div>
...
const state = signal<LoadState<UserDto[]>>({ status: 'loading' });`,
      language: "typescript",
      diagram: "<div class=\"diagram-wrap\"><p class=\"text-center text-xs font-bold text-slate-400 uppercase tracking-widest mb-5\">Where Strict TypeScript Catches Bugs</p><div class=\"grid grid-cols-1 md:grid-cols-3 gap-3 text-xs\"><div class=\"bg-indigo-50 border-2 border-indigo-200 rounded-xl p-3 text-center\"><p class=\"font-bold text-indigo-700\">strictNullChecks</p><p class=\"text-slate-500 mt-1\">null/undefined bugs</p></div><div class=\"bg-emerald-50 border-2 border-emerald-200 rounded-xl p-3 text-center\"><p class=\"font-bold text-emerald-700\">strictTemplates</p><p class=\"text-slate-500 mt-1\">template type mismatches</p></div><div class=\"bg-amber-50 border-2 border-amber-200 rounded-xl p-3 text-center\"><p class=\"font-bold text-amber-700\">input.required()</p><p class=\"text-slate-500 mt-1\">missing required inputs</p></div></div></div>"
    },
    {
      ...
      "explanation": `
        <div class="analogy-box">
          ...
          <div class="analogy-body">
            <p class="analogy-label">Think of it like</p>
            <p class="analogy-text">A building inspector who reviews the blueprint before construction starts, not after the walls are up. Angular's compiler reads TypeScript's type information and decorator metadata at build time — the same way a blueprint gets checked against code before anyone pours concrete — so wiring mistakes (a service that doesn't exist, a template reading a property that isn't there) are caught long before the app ever runs in a browser.</p>
          </div>
        </div>
      ...
      `,
      "code": "// tsconfig.json — strict mode (Angular default)\n{\n  \"compilerOptions\": {\n    \"strict\": true,               // enables all strict TS checks\n    \"strictNullChecks\": true,     // null/undefined are not assignable to other types\n    \"strictPropertyInitialization\": true  // class properties must be initialized\n  },\n  \"angularCompilerOptions\": {\n    \"strictTemplates\": true,      // type-checks template expressions\n    \"strictInjectionParameters\": true  // catches missing @Injectable providers\n  }\n}\n\n// ---- How TypeScript types flow into Angular ----\nimport { Component, inject, signal } from '@angular/core';\nimport { UserService } from './user.service';\n\ninterface User { id: number; name: string; email: string; }\n\n@Component({\n  selector: 'app-profile',\n  template: `\n    <!-- strictTemplates catches: user might be null -->\n    @if (user()) {\n      <!-- Now TypeScript knows user() is User (not null) inside the @if -->\n      {{ user()!.name }}\n    }\n  `\n})\nexport class ProfileComponent {\n  private userService = inject(UserService);\n  // TypeScript knows this is User | null — strictNullChecks enforces handling it\n  user = signal<User | null>(null);\n\n  constructor() {\n    this.userService.getCurrent().subscribe(u => this.user.set(u));\n  }\n}",
      "language": "typescript",
      "diagram": "<div class=\"diagram-wrap\"><p class=\"text-center text-xs font-bold text-slate-400 uppercase tracking-widest mb-5\">Decorator Metadata &rarr; Compiled Output</p><div class=\"flex flex-wrap items-center justify-center gap-2 text-xs\"><div class=\"bg-indigo-50 border-2 border-indigo-200 rounded-lg px-3 py-2 font-semibold text-indigo-700\">@Component({...})</div><span class=\"text-slate-300\">&rarr;</span><div class=\"bg-emerald-50 border-2 border-emerald-200 rounded-lg px-3 py-2 font-semibold text-emerald-700\">Angular compiler reads metadata</div><span class=\"text-slate-300\">&rarr;</span><div class=\"bg-amber-50 border-2 border-amber-200 rounded-lg px-3 py-2 font-semibold text-amber-700\">Optimized JS + type-checked template</div></div></div>"
    },
    {
      ...
      "explanation": `
        <div class="analogy-box">
          ...
          <div class="analogy-body">
            <p class="analogy-label">Think of it like</p>
            <p class="analogy-text">A vending machine that only accepts a fixed list of coin shapes (a union type) versus a machine that just says "insert something" and hopes for the best (<code>any</code>). Utility types like <code>Partial&lt;T&gt;</code> and <code>Pick&lt;T, K&gt;</code> are pre-cut coin templates — instead of hand-cutting a brand-new coin shape for every slightly different slot, you stamp out a variation of a shape you already trust.</p>
          </div>
        </div>
      ...
      "language": "typescript",
      "diagram": "<div class=\"diagram-wrap\"><p class=\"text-center text-xs font-bold text-slate-400 uppercase tracking-widest mb-5\">One Type, Many Derived Shapes</p><div class=\"flex flex-col items-center gap-2 text-xs\"><div class=\"bg-slate-800 text-white rounded-lg px-3 py-1.5\">Product</div><div class=\"grid grid-cols-3 gap-2 mt-2\"><div class=\"bg-indigo-50 border border-indigo-200 rounded-lg p-2 text-center font-semibold text-indigo-700\">CreateProductDto = Omit&lt;Product, 'id'|'createdAt'&gt;</div><div class=\"bg-emerald-50 border border-emerald-200 rounded-lg p-2 text-center font-semibold text-emerald-700\">UpdateProductDto = Partial&lt;...&gt; &amp; {id}</div><div class=\"bg-amber-50 border border-amber-200 rounded-lg p-2 text-center font-semibold text-amber-700\">ProductViewModel = Readonly&lt;Pick&lt;...&gt;&gt;</div></div></div></div>"
    },
    {
      ...
      "explanation": `
        <div class="analogy-box">
          ...
          <div class="analogy-body">
            <p class="analogy-label">Think of it like</p>
            <p class="analogy-text">Customs declaration stickers on shipping crates. A crate with a "@Component" sticker gets routed to the display floor; a crate with an "@Injectable" sticker gets routed to the shared warehouse anyone can request from. At build time Angular reads every sticker, figures out the correct routing plan, and then discards the sticker itself — the final shipped bundle just has direct, pre-routed function calls, no paperwork left attached.</p>
          </div>
        </div>
        ...
        <p>TypeScript decorators that use parameter metadata (like the DI system's constructor injection) historically required the <code>reflect-metadata</code> polyfill and <code>emitDecoratorMetadata: true</code> in <code>tsconfig.json</code>. The <code>inject()</code> function does not need decorator metadata at all, and is the settled modern approach — new Angular code should prefer <code>inject()</code> inside class fields or lifecycle hooks over constructor injection.</p>
      `,
      "code": "import { Component, input, output,\n         HostListener, HostBinding } from '@angular/core';\n\n// ---- Class decorator: configures the component ----\n@Component({\n  selector: 'app-rating',\n  template: `\n    <div class=\"stars\">\n      @for (star of stars; track star) {\n        <span (click)=\"setRating(star)\"\n              [class.filled]=\"star <= value()\">\n          ★\n        </span>\n      }\n    </div>\n  `\n})\nexport class RatingComponent {\n  stars = [1, 2, 3, 4, 5];\n\n  // ---- Signal-based inputs/outputs: communicate with parent ----\n  value = input(0);\n  valueChange = output<number>();\n\n  // ---- Host decorators: bind to the host element ----\n  @HostBinding('attr.role') role = 'slider';\n  @HostBinding('attr.aria-valuenow') get ariaValue() { return this.value(); }\n\n  @HostListener('keydown.arrowRight')\n  increment() { this.setRating(Math.min(5, this.value() + 1)); }\n\n  @HostListener('keydown.arrowLeft')\n  decrement() { this.setRating(Math.max(0, this.value() - 1)); }\n\n  setRating(star: number): void {\n    this.valueChange.emit(star);\n  }\n}\n\n// ---- inject() instead of constructor injection ----\nimport { Injectable, inject } from '@angular/core';\nimport { HttpClient } from '@angular/common/http';\n\n@Injectable({ providedIn: 'root' })\nexport class ProductService {\n  // No constructor, no @Inject, no reflect-metadata needed\n  private http = inject(HttpClient);\n\n  getAll() { return this.http.get<Product[]>('/api/products'); }\n}",
      "language": "typescript",
      "diagram": "<div class=\"diagram-wrap\"><p class=\"text-center text-xs font-bold text-slate-400 uppercase tracking-widest mb-5\">Decorator Metadata Lifecycle</p><div class=\"flex flex-wrap items-center justify-center gap-2 text-xs\"><div class=\"bg-indigo-50 border-2 border-indigo-200 rounded-lg px-3 py-2 font-semibold text-indigo-700\">Source: @Component({...})</div><span class=\"text-slate-300\">AOT compile &rarr;</span><div class=\"bg-emerald-50 border-2 border-emerald-200 rounded-lg px-3 py-2 font-semibold text-emerald-700\">Direct function calls</div><span class=\"text-slate-300\">&rarr;</span><div class=\"bg-slate-100 border border-slate-300 rounded-lg px-3 py-2 font-semibold text-slate-500\">No decorator metadata in final bundle</div></div></div>"
    },
    {
      ...
      "explanation": `
        <div class="analogy-box">
          ...
          <div class="analogy-body">
            <p class="analogy-label">Think of it like</p>
            <p class="analogy-text">A job contract versus a recipe. An interface is a job contract — a role a class can formally sign and agree to fulfil (<code>implements</code>), and multiple contracts can be merged or extended over time. A type alias is a recipe — perfect for describing a mix of ingredients (unions, intersections, tuples) that no one is expected to "sign up to," just to follow.</p>
          </div>
        </div>
        ...
        <p>Interface declaration merging — declaring the same interface name twice and having TypeScript merge them — is useful when augmenting third-party library types. For example, adding a custom property to Express's <code>Request</code> interface in an SSR server. Type aliases do not support this.</p>
      `,
      ...
      "language": "typescript",
      "diagram": "<div class=\"diagram-wrap\"><p class=\"text-center text-xs font-bold text-slate-400 uppercase tracking-widest mb-5\">interface vs type — Quick Rule</p><div class=\"grid grid-cols-1 md:grid-cols-2 gap-4 text-xs\"><div class=\"bg-indigo-50 border-2 border-indigo-200 rounded-xl p-3\"><p class=\"font-bold text-indigo-700 text-center mb-2\">interface</p><p class=\"text-slate-600 text-center\">class implements it, extends other interfaces, merges declarations</p></div><div class=\"bg-emerald-50 border-2 border-emerald-200 rounded-xl p-3\"><p class=\"font-bold text-emerald-700 text-center mb-2\">type</p><p class=\"text-slate-600 text-center\">unions, intersections, tuples, mapped/conditional types</p></div></div></div>"
    },
    {
      ...
      "explanation": `
        <div class="analogy-box">
          ...
          <div class="analogy-body">
            <p class="analogy-label">Think of it like</p>
            <p class="analogy-text"><code>satisfies</code> is a fitting-room mirror: it confirms the outfit (your object) matches the dress code (a type) without altering the outfit itself — you keep the exact fabric and buttons (literal types), you just get confirmation it's compliant. <code>infer</code> is a detective extracting one clue (the wrapped type) from inside a sealed envelope (a generic wrapper like <code>Observable&lt;T&gt;</code>) without needing to open the whole case file.</p>
          </div>
        </div>
        ...
        <p>Template literal types (<code>\`on${'$'}{string}\`</code>) let you type event handler names, CSS class combinations, or API endpoint patterns at the type level.</p>
      ...
      "language": "typescript",
      "diagram": "<div class=\"diagram-wrap\"><p class=\"text-center text-xs font-bold text-slate-400 uppercase tracking-widest mb-5\">satisfies vs a Plain Annotation</p><div class=\"grid grid-cols-1 md:grid-cols-2 gap-4 text-xs\"><div class=\"bg-rose-50 border-2 border-rose-200 rounded-xl p-3\"><p class=\"font-bold text-rose-700 text-center mb-2\">: AppConfig</p><p class=\"text-slate-600 text-center\">widens values, loses literal types</p></div><div class=\"bg-emerald-50 border-2 border-emerald-200 rounded-xl p-3\"><p class=\"font-bold text-emerald-700 text-center mb-2\">satisfies AppConfig</p><p class=\"text-slate-600 text-center\">checks shape, KEEPS literal types</p></div></div></div>"
    },
    {
      ...
      "explanation": `
        <div class="analogy-box">
          ...
          <div class="analogy-body">
            <p class="analogy-label">Think of it like</p>
            <p class="analogy-text">Duplicating a spreadsheet by copying just the top sheet while every embedded chart still links back to the original workbook's data. That's a shallow copy — edit the "duplicate," and the linked chart in the "original" changes too, because it was never actually duplicated, just the outer file.</p>
          </div>
        </div>
      ...
      "language": "typescript",
      "diagram": "<div class=\"diagram-wrap\"><p class=\"text-center text-xs font-bold text-slate-400 uppercase tracking-widest mb-5\">Copy Only the Changed Path</p><div class=\"flex flex-col items-center gap-1 text-xs font-mono\"><div class=\"bg-indigo-50 border border-indigo-200 rounded px-3 py-1\">products (new array)</div><div class=\"text-slate-300\">&darr;</div><div class=\"bg-emerald-50 border border-emerald-200 rounded px-3 py-1\">this product (new object)</div><div class=\"text-slate-300\">&darr;</div><div class=\"bg-amber-50 border border-amber-200 rounded px-3 py-1\">meta (new object)</div><p class=\"text-slate-400 mt-2 text-[11px]\">other products in the array keep their old reference — untouched, no wasted work</p></div></div>"
    },
    {
      ...
      "explanation": `
        <div class="analogy-box">
          ...
          <div class="analogy-body">
            <p class="analogy-label">Think of it like</p>
            <p class="analogy-text">Handing someone a photocopy of a number written on paper (a primitive) versus handing them the key to your actual apartment (a reference). Scribble on the photocopy and your original paper is untouched. But if you and a roommate share the same apartment key, and your roommate repaints a wall, you walk in to find it repainted too — because you never had "your own copy" of the apartment, just another key to the same place.</p>
          </div>
        </div>
      ...
      `,
      "code": "interface User {\n  id: number;\n  name: string;\n  address: { city: string };\n}\n\n// Primitive copy: independent value.\nlet a = 10;\nlet b = a;\nb = 20;\nconsole.log(a); // 10\n\n// Reference copy: both variables point to the same object.\nconst user: User = { id: 1, name: 'Asha', address: { city: 'Pune' } };\nconst sameUser = user;\nsameUser.name = 'Neha';\nconsole.log(user.name); // 'Neha'\n\n// Shallow copy: outer object is new, nested address is still shared.\nconst shallowUser = { ...user };\nshallowUser.address.city = 'Mumbai';\nconsole.log(user.address.city); // 'Mumbai'\n\n// Copy the nested path to avoid leaking changes.\nconst updatedUser: User = {\n  ...user,\n  address: {\n    ...user.address,\n    city: 'Bengaluru'\n  }\n};\n\n// Angular input pattern: do not mutate input() objects directly.\n// Emit an event or call a service that creates the next immutable state.",
      "language": "typescript",
      "diagram": "<div class=\"diagram-wrap\"><p class=\"text-center text-xs font-bold text-slate-400 uppercase tracking-widest mb-5\">Value Copy vs Reference Copy</p><div class=\"grid grid-cols-1 md:grid-cols-2 gap-4 text-xs\"><div class=\"bg-emerald-50 border-2 border-emerald-200 rounded-xl p-3\"><p class=\"font-bold text-emerald-700 text-center mb-2\">let b = a (primitive)</p><p class=\"text-slate-600 text-center\">b is an independent copy</p></div><div class=\"bg-rose-50 border-2 border-rose-200 rounded-xl p-3\"><p class=\"font-bold text-rose-700 text-center mb-2\">const sameUser = user (object)</p><p class=\"text-slate-600 text-center\">sameUser points to the SAME object</p></div></div></div>"
    },
    {
      ...
      "explanation": `
        <div class="analogy-box">
          ...
          <div class="analogy-body">
            <p class="analogy-label">Think of it like</p>
            <p class="analogy-text">Editing a shared Google Doc by making a copy and proposing changes in the copy, instead of typing directly over someone else's cursor position in the live doc. <code>[...items, newItem]</code> is "duplicate the doc, then add my paragraph" — everyone still watching the original sees it cleanly swap to the new version at a known moment, instead of watching text change under their cursor.</p>
          </div>
        </div>
      ...
      "language": "typescript",
      "diagram": "<div class=\"diagram-wrap\"><p class=\"text-center text-xs font-bold text-slate-400 uppercase tracking-widest mb-5\">Five Immutable Operations</p><div class=\"grid grid-cols-1 md:grid-cols-5 gap-2 text-xs\"><div class=\"bg-indigo-50 border border-indigo-200 rounded-lg p-2 text-center font-semibold text-indigo-700\">Add</div><div class=\"bg-emerald-50 border border-emerald-200 rounded-lg p-2 text-center font-semibold text-emerald-700\">Remove</div><div class=\"bg-amber-50 border border-amber-200 rounded-lg p-2 text-center font-semibold text-amber-700\">Update one</div><div class=\"bg-rose-50 border border-rose-200 rounded-lg p-2 text-center font-semibold text-rose-700\">Sort</div><div class=\"bg-purple-50 border border-purple-200 rounded-lg p-2 text-center font-semibold text-purple-700\">Patch</div></div></div>"
    }
  ]
});
