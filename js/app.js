/* =============================================================
   Angular Tutorial â€” app.js
   State · Rendering · Navigation
================================================================ */
...
let state = {
  moduleId:   null,
  questionId: null,
  done:       JSON.parse(localStorage.getItem('at_done') || '{}'),
};

// -- Flat question index for prev/next ----------------------
function buildIndex() {
  const idx = [];
  MODULES.forEach(m => m.questions.forEach(q => idx.push({ moduleId: m.id, questionId: q.id })));
  return idx;
}
...
function renderModuleIcon(mod, sizeClass = 'text-base') {
...
function markDone(mId, qId) {
  state.done[doneKey(mId, qId)] = true;
  localStorage.setItem('at_done', JSON.stringify(state.done));
  updateProgress();
  // update dot colour in sidebar
...
function renderSidebar() {
  ...
  MODULES.forEach(mod => {
    ...
    btn.onclick = () => toggleModule(mod.id);
    ...
    mod.questions.forEach(q => {
      ...
      qb.className = 'q-btn';
      qb.dataset.qid = q.id;
      const isDone = state.done[doneKey(mod.id, q.id)];
      if (isDone) qb.classList.add('done');

      qb.innerHTML = `<span class="q-dot"></span><span class="flex-1 text-left">${q.title}</span>`;
      qb.onclick = () => loadQuestion(mod.id, q.id);
      list.appendChild(qb);
    });
...
function toggleModule(id) {
  ...
  // Close all
  document.querySelectorAll('.question-list').forEach(el => el.classList.remove('open'));
  document.querySelectorAll('.module-btn').forEach(el => el.classList.remove('open'));
...
function loadQuestion(moduleId, questionId) {
  ...
  bc.innerHTML = `
    <span class="text-slate-400">Angular</span>
  ...
  const pos   = idx.findIndex(i => i.moduleId === moduleId && i.questionId === questionId);
  ...
  // Save last position
  localStorage.setItem('at_last', JSON.stringify({ moduleId, questionId }));
}
...
function renderWelcome() {
  ...
  area.innerHTML = `
    <div class="max-w-3xl mx-auto px-5 lg:px-8 py-10">
      ...
      <!-- Hero -->
      <div class="content-card p-8 mb-6 bg-gradient-to-br from-slate-900 to-indigo-900 text-white border-0">
        <div class="flex items-center gap-4 mb-4">
          <div class="w-12 h-12 bg-red-500 rounded-xl flex items-center justify-center shadow-lg flex-shrink-0">
            <svg class="w-7 h-7 text-white" viewBox="0 0 250 250" fill="currentColor">
              <path d="M125 30L31.9 63.2l14.2 123.1L125 230l78.9-43.7 14.2-123.1z"/>
              <path fill="rgba(255,255,255,0.3)" d="M125 30v200l78.9-43.7 14.2-123.1L125 30z"/>
              <path fill="white" d="M125 52.1L66.8 182.6h21.7l11.7-29.2h49.4l11.7 29.2H183L125 52.1zm17 83.3h-34l17-40.9 17 40.9z"/>
            </svg>
          </div>
          <div>
            <h1 class="text-2xl font-bold">Angular Tutorial</h1>
            <p class="text-slate-300 text-sm mt-0.5">Interview Preparation Guide</p>
          </div>
        </div>
        <p class="text-slate-300 text-sm leading-relaxed">A complete, module-wise guide covering Angular concepts with explanations, code examples, and visual diagrams. Pick a topic from the sidebar to get started.</p>
        <div class="flex gap-6 mt-5">
          <div class="text-center">
            <p class="text-2xl font-bold text-white">${MODULES.length}</p>
            <p class="text-xs text-slate-400">Modules</p>
          </div>
          <div class="text-center">
          ...
          <div class="text-center">
            <p class="text-2xl font-bold text-indigo-300">${Object.values(state.done).filter(Boolean).length}</p>
      ...
      <div class="grid grid-cols-1 gap-3">
        ${MODULES.map(mod => {
          const done = mod.questions.filter(q => state.done[doneKey(mod.id, q.id)]).length;
          const pct  = mod.questions.length ? Math.round((done / mod.questions.length) * 100) : 0;
          return `
            <div class="welcome-module-card" onclick="loadQuestion('${mod.id}','${mod.questions[0].id}')">
              <div class="flex items-center gap-3">
                ...
                <div class="flex-1 min-w-0">
                  ...
                  <p class="text-xs text-slate-400">${mod.questions.length} questions · ${done} done</p>
                </div>
...
function init() {
  ...
  // Restore last position or show welcome
  const last = JSON.parse(localStorage.getItem('at_last') || 'null');
  if (last && getModule(last.moduleId)) {
